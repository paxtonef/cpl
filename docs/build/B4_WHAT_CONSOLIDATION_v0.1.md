# CPL — B4 WHAT Consolidation v0.1

**System:** Common Product Layer — CPL
**Phase:** B4 — Assets + Relationships
**Artifact:** WHAT Consolidation
**Version:** v0.1
**Status:** REPAIRED — PROPOSED FOR GLOBAL RE-CHALLENGE
**Repair source:** `B4_WHAT_GLOBAL_FREEZE_CHALLENGE_v0` (REPAIR_REQUIRED)
**Canonical baseline:** `main @ 312d7fc`
...
`B4_WHAT_CONSOLIDATION_v0` was taken through the Global WHAT Freeze Challenge. The core architecture, the Asset Authority submodel, and the Relationship submodel all survived unchanged (`WHAT_REOPEN_REQUIRED = NO`). Two bounded consolidation-closure defects were found and authorized for repair:

```text
FR-B4-01  Survivor-selection determinacy   (repairs §12–§15)
FR-B4-02  Identity-operation authority boundary   (repairs §50)
```

v0.1 preserves the entirety of Consolidation v0 and introduces **only** these two repairs, plus the two invariant clarifications the challenge required (`B4-CI31`, `B4-CI32` — see §72 and §75). No other section is altered. In particular, this repair does **not** reopen:

```text
Asset Authority allocation
Relationship Authority allocation
...
relationship type envelope
ExternalReference semantics
DomainProjection semantics
```

and introduces no numeric survivor score, no automatic domain-resolver survivor selection, no new first-class semantic object, no new relationship vocabulary, no new authorization behavior, and no implementation architecture.
...
## 12. C-B4-01 — Asset survivor selection [REPAIRED — FR-B4-01]
...
The Global WHAT Freeze Challenge (`FC-B4-06`) found that the v0 "maximizes preservation of existing canonical continuity" formulation, while directionally correct, was not sufficiently determinate: two developers could reasonably reach opposite survivor conclusions from the same facts, each citing the consolidated WHAT. That is a freeze-blocking defect. v0.1 replaces the general principle with a governed precedence order.

---

## 13. Survivor-selection principle — DEFAULT SURVIVOR PRECEDENCE

The survivor is not selected by weighing factors or by an implicit or numeric score. It is selected by applying the following precedence order, in sequence, stopping at the first rule that determines a result:

```text
1. Existing governing canonical successor/survivor
   Where an existing canonical successor/survivor already governs
   one of the candidate Assets (e.g. from a prior merge or
   correction), that established canonical successor/survivor
   is preferred.

2. Established canonical continuity over later duplicate
   Otherwise, the Asset with established CPL canonical continuity
   — existing canonical role, historical continuity, established
   external references, current canonical references, accepted
   domain projections, existing cases/operational continuity,
   identity-resolution history — is preferred over a later
   duplicate representation.

3. Override
   The default survivor determined by rules 1–2 may be overridden
   ONLY where:
     - that default Asset is proven canonically inadmissible; or
     - an explicit governed CPL continuity rule requires another
       survivor.

4. Override durability
   Any override under rule 3 MUST be represented, with its reason,
   in the CanonicalAssetIdentityDecision. An override without a
   durable recorded reason is not a valid survivor selection.

5. Domain resolver authority does not extend to survivor selection
   Physical-identity resolver preference (e.g. a VIR resolution)
   MUST NOT itself select the CPL survivor. Domain resolver output
   may inform admissibility and evidence; it does not carry
   CPL canonical-continuity authority unless a specific CPL policy
   has separately authorized that input to survivor selection.
```

This is a normative hierarchy, not a numeric score. It removes the ambiguity the challenge identified while leaving HOW the rule is evaluated (e.g. exact query shape, tie-break implementation detail within an already-determinate branch) to the Requirement Matrix and downstream implementation.

---

## 14. Existing canonical continuity preference

Rule 2 of §13 preserves the original v0 intent: where one Asset is already the established current canonical representation and the other is a later duplicate representation, the existing canonical Asset survives unless rule 1 already determined a different governing successor, or rule 3's override conditions are met and durably recorded. Thus:
...
prefer established canonical continuity
    (unless rule 1 or a recorded rule 3 override applies)
```

This is a governed default within a determinate precedence order — not, as in v0, a standalone default that left the boundary of "material governance reason" open to interpretation.

---

## 15. Survivor override

A different survivor may be required where the existing canonical Asset is itself structurally or semantically unsuitable, per §13 rule 3. Any override MUST be explicit and durably traceable through the `CanonicalAssetIdentityDecision`, per §13 rule 4. An override reason that is not recorded in the `CanonicalAssetIdentityDecision` does not authorize a deviation from the default survivor.

### C-B4-01 status

```text
RESOLVED AT WHAT LEVEL — REPAIRED (v0.1, FR-B4-01)
```

Implementation still chooses how to evaluate each rule (e.g. how "established canonical continuity" is queried); it does not choose whether or in what order the rules apply.
...
## 50. C-B4-07 — Final B4 operation surface [REPAIRED — FR-B4-02]

The consolidated B4 operation surface should be semantic rather than transport-specific.

The Global WHAT Freeze Challenge (`FC-B4-15A`) found that `assess_asset_identity`, as named in v0, could reasonably be read by an implementation team as authority for generic CPL to itself perform domain physical-identity determination. That would contradict the stabilized doctrine (§6, §69): domain authority (e.g. VIR) determines physical identity; CPL consumes, admits and governs that determination but does not produce it. v0.1 replaces that operation with a family whose names cannot be misread that way.
...
```text
request_asset_identity_resolution
    (request/consume domain physical-identity determination;
     does not itself determine physical identity)

record_asset_identity_resolution
    (persist a domain-produced AssetIdentityResolution)

evaluate_asset_resolution_admissibility
    (governed CPL check of whether a recorded resolution is
     admissible for canonical admission; not a physical-identity
     determination)
...
The normative point, per `B4-CI32` (§75), is invariant across any exact downstream naming: DOMAIN produces the identity determination; CPL consumes, records and governs the admissibility and canonical effect of that determination. No operation in the generic B4 surface may itself perform domain physical-identity determination. Exact method/endpoint names remain a Requirement Matrix concern.
...
```text
RESOLVED AT WHAT LEVEL — REPAIRED (v0.1, FR-B4-02)
```
...
B4-CI31 Asset merge survivor selection MUST follow governed CPL
        canonical-continuity precedence and MUST NOT be delegated
        to arbitrary implementation preference. [ADDED — v0.1]
```
...
B4-CI32 Generic CPL operations MAY request, consume, persist and
        govern admissibility of domain identity determinations but
        MUST NOT implicitly acquire the domain authority that
        produces those determinations. [ADDED — v0.1]
```
...
```text
Global Freeze Challenge v0 defects:

FR-B4-01  Survivor-selection determinacy
  APPLIED (v0.1)

FR-B4-02  Identity-operation authority boundary
  APPLIED (v0.1)
```
...
B4 WHAT Consolidation v0.1
  PRODUCED
  REPAIRS APPLIED: FR-B4-01, FR-B4-02
  PROPOSED FOR GLOBAL RE-CHALLENGE
...
B4_WHAT_CONSOLIDATION_v0
      ↓
B4 WHAT GLOBAL FREEZE CHALLENGE v0
      ↓
REPAIR_REQUIRED (FR-B4-01, FR-B4-02)
      ↓
B4_WHAT_CONSOLIDATION_v0.1  ← this artifact
      ↓
materialization
      ↓
B4 WHAT GLOBAL FREEZE RE-CHALLENGE v0.1
      ↓
   ┌──┴─────────────┐
   │                │
...
Global Freeze Challenge v0 result:
  REPAIR_REQUIRED
  WHAT_REOPEN_REQUIRED = NO

Repairs applied in v0.1:
  FR-B4-01  Survivor-selection determinacy
  FR-B4-02  Identity-operation authority boundary

New invariants added in v0.1:
...
READY FOR:
  GLOBAL B4 WHAT FREEZE RE-CHALLENGE v0.1
...
**END — B4 WHAT Consolidation v0.1**
